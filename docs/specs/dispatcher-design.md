...
Dispatcher 是 linen 的客户端执行器。它负责：

1. 协调项目容器生命周期
2. 给 Agent 下发明确任务
3. 代 Agent 调用 linen API 写回图

Agent 不直接认领 Intent，不直接 heartbeat，不直接调用 linen API。Agent 只接收 Dispatcher 下发的任务，返回结构化结果；Dispatcher 再决定如何请求 Server。

---

## 设计要点

1. Agent 的执行任务收敛成四类：`bootstrap`、`reason`、`explore` 和 `review`；`bootstrap` 只在项目初始态运行，让 Agent 直接尝试解决整个问题；主阶段可返回进展 `fact`，仅在确实解决时同时返回 `complete`；若主阶段超时，再由 `bootstrap_conclude` 收尾产出 Fact；`reason` 负责读图判断是否完成或是否需要提出一个新 intent；`explore` 只负责执行一个已认领 intent 并产出一个 Fact 结论；`review` 对一个 draft Fact 追加独立 Review，并结束对应 review Intent。
2. Dispatcher 是唯一的协议写入者和控制面；Agent 不 claim、不 heartbeat、不直接调用 linen API。
...
5. 调度上，项目初始态按 `project.bootstrap_enabled` 和 Worker 能力决定优先 `bootstrap` 或直接 `reason`；非初始态的 `graph_revision` 出现新版本时优先 `reason`，否则优先消费可认领的 `explore` intent；`reason` 的并发约束通过服务端的项目级 `project.reason` lease 表达为“单项目最多一个”，每次运行还使用唯一 `lease_id` 隔离同名 Worker；`bootstrap` 的并发约束是“单项目最多一个保留 bootstrap intent 且最多一个 bootstrap 任务”，跨项目允许并行；`runtime.interval` 被刻意复用为主循环节拍和带 claim 任务的 heartbeat 周期。
...
13. `scope` 审计的固定步骤不保存在 Dispatcher 私有队列中。Dispatcher 每轮从
...
4. 如果是 `explore`，Dispatcher 先通过 `POST /projects/{project_id}/intents/{intent_id}/heartbeat` 认领目标 intent；如果是 `reason`，则生成本次运行唯一的 `lease_id`，再通过 `POST /projects/{project_id}/reason/claim` 认领项目级 reason lease
...
| 任务 | 触发条件 | 输入 | 输出 | 超时策略 |
| --- | --- | --- | --- | --- |
| `bootstrap` | 项目 `active`；`project.bootstrap_enabled=true`，且配置中存在支持 `bootstrap` 的 Worker 或项目已经存在保留 bootstrap intent；facts 只有 `origin` 和 `goal`；当前没有普通 intent；允许不存在 bootstrap intent，或只存在保留的 open `bootstrap` intent | `{origin}`、`{goal}`、`{hints}` | 主阶段返回 `fact`，已解决时可附 `complete`；收尾阶段只返回 `fact` | 双阶段：`timeout` 后可进入 `conclude_timeout` 收尾；两阶段都失败则 release 保留 intent，下轮仍按新项目重试 |
...
- 主阶段可以返回一个已证实的进展 `fact`
- 只有已经解决问题时，主阶段才同时包含 `fact` 和 `complete`
- 如果主阶段没有及时返回结构化进展，Dispatcher 会终止它并进入 `bootstrap_conclude`
...
- 主阶段始终要求 `data.fact.description`；携带 `complete` 时还要求 `data.complete.description`
- 主阶段只返回 `fact` 表示记录进展并转交普通 Reason/Explore，不表示项目完成
...
- 主阶段如果在 `timeout` 内产生可靠进展可返回 `fact`；完全解决时返回 `fact + complete`
...
- 如果 `bootstrap` 主阶段成功返回 `fact`，Dispatcher 会 conclude 写入该 Fact；仅当同时返回
  `complete` 时再请求完成项目，且审计项目仍须通过服务端完成门禁
...
- 之后只有 `graph_revision` 增加或项目从“存在 open intents”进入“没有 open intents”才重新触发
- `graph_revision` 覆盖 Fact、Intent、Hint、Review、Fact 状态和项目元数据等语义写入，因此 Review-only 变化也不会被漏掉
...
4. `reason` 的去重按服务端单调递增的 `graph_revision` 做：首次只有在当前没有任何 open intent 时才触发；之后只有出现新的黑板语义版本，或项目从“存在 open intents”进入“没有 open intents”时，才重新触发
...
- 但前提不是“上一次 reason 刚写入了 intent”：成功收尾时 checkpoint 从服务端重新读取，因此会吸收该次 Reason 自己产生的 revision，不会立即自触发
- 仍然要求当前没有未认领 intent、当前项目内没有其他 `reason` 任务在运行、且没有超过 `runtime.max_project_workers`

`reason` 的去重规则建议保持简单：

- 记录该项目上次成功完成 `reason` 后重新读取到的 `graph_revision`，并保留 Fact、Hint、Review 与 open intent 计数作为诊断信息
- Dispatcher 对“当前已观察到、且已有 open intents、但尚无 checkpoint 的 active 项目”建立基线 checkpoint；启动时已有项目和运行中晚到项目都适用，不应吞掉运行过程中第一次新的语义写入
- 首次没有历史记录且当前没有 open intents 时，直接触发
- 之后只有 `graph_revision` 增加，或项目从“有 open intents”变为“无 open intents”时，才再次触发 `reason`
- Reason 成功后的 checkpoint 必须读取写回后的项目状态，避免把 Reason 自己创建的 Intent 误判成外部新态势
...
- 单个项目内，同一时刻最多只能有一个 `reason` 任务在运行；Dispatcher 在本地运行表和服务端 lease 两层去重
- 每次 `reason` 运行生成唯一 `lease_id`；heartbeat/release 必须同时匹配 Worker 名和令牌，防止同名 Worker 的旧运行释放新租约
- 跨项目允许并行运行多个 `reason`
...
codex exec --dangerously-bypass-approvals-and-sandbox --model "{env.CODEX_MODEL}" \
  -c 'model_provider="linen"' \
  -c 'model_providers.linen.name="linen"' \
  -c 'model_providers.linen.wire_api="responses"' \
  -c 'model_reasoning_effort="high"' \
  -c 'model_providers.linen.base_url="{env.CODEX_BASE_URL}"' \
  -c 'model_providers.linen.env_key="OPENAI_API_KEY"' \
  -- "{prompt}"
```
...
| 字段 | 必填 | 含义 |
| --- | --- | --- |
| `server` | 是 | linen Server 的 base URL |
...
- 如果项目已从 Server 删除，Dispatcher 会把找不到对应项目的 `linen-dispatch-*` 容器视为 orphan，并执行 stop 清理
