...
from linen.dispatcher.models import AuditGraphCheckpoint
...
def test_audit_graph_model_validator_accepts_only_semantic_current_revision(tmp_path):
    board = project_detail = ProjectDetail(
        project=ProjectMeta(
            id="proj_001", title="audit", status="active", bootstrap_enabled=False,
            audit_mode="hypothesis", graph_revision=7, created_at="2026-01-01T00:00:00Z",
        ),
        facts=[
            Fact(id="origin", description="repo"),
            Fact(id="goal", description="audit"),
            Fact(id="f001", description="request source", type="source", status="triaged"),
        ],
        intents=[], hints=[], reviews=[],
    )
    ...
    with pytest.raises(ValueError, match="cannot complete"):
        audit_graph.validate_model_intents(
            {"accepted": True, "data": {"complete": {"from": ["f001"]}}},
            project_detail,
            expected_revision=7,
            max_intents=3,
        )
...
def test_audit_graph_model_trigger_is_bounded_per_revision(tmp_path):
    cfg = config(tmp_path, audit=True, mode="hypothesis")
    cfg.audit.graph_reason.enabled = True
    ...
    board = ProjectDetail(
        project=ProjectMeta(
            id="proj_001", title="audit", status="active", bootstrap_enabled=False,
            audit_mode="hypothesis", graph_revision=4, created_at="2026-01-01T00:00:00Z",
        ),
        facts=[Fact(id="origin", description="repo"), Fact(id="goal", description="audit")],
        intents=[], hints=[], reviews=[],
    )
    assert loop._audit_graph_reason_trigger(board) == "audit_graph:revision:4:attempt:1"
    loop.audit_graph_checkpoints[board.project.id] = AuditGraphCheckpoint(4, 1)
    assert loop._audit_graph_reason_trigger(board) == "audit_graph:revision:4:attempt:2"
    loop.audit_graph_checkpoints[board.project.id] = AuditGraphCheckpoint(4, 2)
    assert loop._audit_graph_reason_trigger(board) is None
    board.project.graph_revision = 5
    assert loop._audit_graph_reason_trigger(board) == "audit_graph:revision:5:attempt:1"
