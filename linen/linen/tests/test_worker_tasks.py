...
def test_reason_treats_retry_of_already_ready_intent_as_idempotent(monkeypatch) -> None:
    config = make_config()
    intent = make_intent()
    intent.worker = None
    project = make_project(intents=[intent])
    client = FakeClient(project)
    client.resolve_intent = lambda *_args, **_kwargs: ApiResult(
        409, {"detail": "Intent has no unresolved blocked error"},
    )
    containers = FakeContainerManager()
    driver = FakeDriver()
    lease = FakeLease()
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: ProcessResult(
            0,
            '{"accepted":true,"data":{"resolve":[{"intent_id":"i001","action":"retry"}]}}',
            "",
        ),
    )
...
def test_reason_complete_ignores_global_audit_for_non_audit_project(monkeypatch) -> None:
    config = make_config()
    config.audit.enabled = True
    project = make_project()
