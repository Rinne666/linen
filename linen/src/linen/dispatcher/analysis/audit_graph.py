...
MODEL_CREATOR = "dispatcher.audit-graph-model"
...
MODEL_INTENT_TYPES = frozenset({
    ...
    "review:cold-verifier",
    "review:contradiction-reasoner",
})


def validate_model_intents(
    payload: dict[str, Any],
    project: ProjectDetail,
    *,
    expected_revision: int,
    max_intents: int,
    skill_choices: list[dict[str, Any]] | None = None,
) -> tuple[str, list[dict[str, Any]]]:
    """Validate a model proposal without granting it graph-write authority.

    The prompt is advisory. This validator is the security boundary: it
    rejects completion, reserved mechanical work, stale graph reads, unknown
    fact references and duplicate or structurally invalid proposals.
    """
    if project.project.graph_revision != expected_revision:
        raise ValueError(
            f"stale graph revision {expected_revision}; current revision is "
            f"{project.project.graph_revision}"
        )
    ...
    if accepted is False:
        return "rejected", []
    if set(payload) != {"accepted", "data"} or not isinstance(payload.get("data"), dict):
        raise ValueError("accepted model output requires exactly one data object")
    data = payload["data"]
    if "complete" in data:
        raise ValueError("audit graph model cannot complete a project")
    if set(data) - {"intents", "skills"}:
        raise ValueError("audit graph model data may only contain intents and skills")
    ...
    if len(raw_skills) > 1:
        raise ValueError("select at most one managed skill per audit-graph pass")
    if len(raw_intents) + len(raw_skills) > max_intents:
    ...
    prior_review_types: dict[str, set[str]] = {}
    for intent in project.intents:
        if len(intent.from_) == 1 and (intent.type or "").startswith("review"):
            prior_review_types.setdefault(intent.from_[0], set()).add(intent.type or "review")
    open_review_ids = {
        intent.from_[0]
        for intent in project.intents
        if len(intent.from_) == 1
    ...
    for index, item in enumerate(raw_intents):
        ...
        if (
            ...
            or not from_ids
            or any(not isinstance(fact_id, str) or fact_id not in facts for fact_id in from_ids)
        ...
        if intent_type.startswith("review"):
            if len(from_ids) != 1 or from_ids[0] in open_review_ids:
                raise ValueError("review intents require one fact without an open review")
            fact_id = from_ids[0]
            prior = sorted(
            ...
            if not prior:
                ...
                if len(prior) >= 2 or _reviewed(project, fact_id):
                    raise ValueError("review retry budget is exhausted or the fact is resolved")
                allowed_followup = (
                    "review:contradiction-reasoner"
                    if any(review.verdict == "NEEDS_REVIEW" for review in prior)
                    else "review:cold-verifier"
                )
                if (
                    intent_type != allowed_followup
                    or intent_type in prior_review_types.get(fact_id, set())
                ):
                    raise ValueError(
    ...
    choices_by_id = {
        ...
        if isinstance(choice, dict) and isinstance(choice.get("skill_id"), str)
    }
    if choices_by_id and not raw_skills:
        raise ValueError("one pending required skill must be selected")
    for index, item in enumerate(raw_skills):
        if not isinstance(item, dict) or set(item) != {"skill_id", "reason"}:
            raise ValueError(
                f"skill at index {index} requires exactly skill_id and reason"
            )
        skill_id = item["skill_id"]
        reason = item["reason"]
        if not isinstance(skill_id, str) or skill_id not in choices_by_id:
            raise ValueError(f"skill at index {index} is not a pending registry choice")
        if not isinstance(reason, str):
            raise ValueError(f"skill at index {index} reason must be text")
        reason = " ".join(reason.split())
        if not 8 <= len(reason) <= 400:
            raise ValueError(f"skill at index {index} reason length is invalid")
