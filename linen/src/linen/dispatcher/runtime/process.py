"""Host subprocess execution for workers.

Local mode is the only execution mode in this build: each project gets an
isolated working directory under ``runtime.local.workspace_root`` and every
worker invocation is a host subprocess spawned from that directory.
"""
...
class ExecProcess(Protocol):
    """Surface every worker process exposes to the task runners.

    The cancellation hook and heartbeat lease rely on this protocol; the local
    implementation lives in :class:`LocalProcess` below.
    """
...
class LocalProcess:
    ...
    Uses its own process group so children are killed as a group, a
    Python-enforced timeout (no ``timeout`` coreutil dependency) and a
    SIGTERM -> grace -> SIGKILL shutdown so the CLI can flush its session
    before dying.
    """
