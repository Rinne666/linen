...
def local_cli_health(executable: str, timeout: float) -> HealthResult:
    """Check a host-configured CLI without requiring or exposing provider keys."""
    path = shutil.which(executable)
    if path is None:
        return HealthResult(False, None, f"{executable} is not on PATH")
    try:
        ...
        return HealthResult(False, None, _clip(str(exc)))
    detail = "" if result.returncode == 0 else _clip(result.stderr or result.stdout)
    return HealthResult(result.returncode == 0, 200 if result.returncode == 0 else None, detail)
