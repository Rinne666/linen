...
from linen.dispatcher.workers.base import DriverResult, WorkerDriver
...
class PiDriver(WorkerDriver):
    ...
    def check_health(self, worker: WorkerConfig, *, timeout: float) -> HealthResult:
        env = worker.env
        if self.local or not {"PI_BASE_URL", "PI_MODEL", "PI_PROVIDER_API", "PI_API_KEY"}.issubset(env):
            return local_cli_health("pi", timeout)
    ...
    def describe_health(self, worker: WorkerConfig) -> str:
        env = worker.env
        if self.local or "PI_BASE_URL" not in env:
            return "pi --help (host CLI configuration)"
    ...
    def _local_argv(self, worker: WorkerConfig, prompt: str, session: str | None) -> list[str]:
        ...
        # We pass --session-id (not --session): --session looks up an existing session file
    ...
    def _agent_dir(worker: WorkerConfig) -> str:
        return str(PurePosixPath("/tmp/linen-pi") / worker.name)
    ...
    def _models_json(worker: WorkerConfig) -> str:
        ...
        payload = {"providers": {"linen": provider}}
