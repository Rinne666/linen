...
from linen.dispatcher.prompting import format_hints, load_prompt, render_prompt
...
def _scope_audit_recon(config: DispatchConfig, project: ProjectDetail) -> bool:
    return (
...
def run_bootstrap_task(
    ...
    try:
        ...
        if audit_recon:
            prompt += (
                "\nAudit reconnaissance mode: this output is non-authoritative. "
        ...
        if provider_failure is not None:
            LOG.warning(
                "bootstrap provider unavailable project=%s intent=%s worker=%s outcome=%s",
        ...
        if not did_timeout(first) and first.returncode == 0:
            ...
            if kind == "rejected":
                ...
                best_effort_release(client, project.project.id, intent.id, worker.name)
                return "rejected"
            return _write_bootstrap_result(
                config,
                client,
                project.project.id,
                intent.id,
...
def _try_conclude_fallback(
    ...
    if provider_failure is not None:
        LOG.warning(
            "bootstrap conclude provider unavailable project=%s intent=%s worker=%s outcome=%s",
            project.project.id, intent.id, worker.name, provider_failure,
        )
        best_effort_release(client, project.project.id, intent.id, worker.name)
        return provider_failure
    ...
    return write_conclude_result(
        ...
        fact_type="recon" if _scope_audit_recon(config, project) else None,
        fact_status="draft",
    )
...
def _write_bootstrap_result(
    ...
    *,
    ...
    audit_recon: bool = False,
) -> str:
    if audit_recon:
        evidence = fact_evidence or "Reconnaissance output; not admissible vulnerability evidence."
        return write_conclude_result(
            client,
            project_id,
            intent_id,
    ...
    if complete_description is None:
        LOG.info(
            "bootstrap recorded progress without completion project=%s intent=%s worker=%s source=%s fact_id=%s",
            project_id,
            intent_id,
            worker_name,
            source,
